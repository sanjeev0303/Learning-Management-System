...
export const useChannelInfo = () => {
  ...
  const inputRef = useRef<HTMLInputElement | null>(null); // Change to HTMLInputElement
  ...
  const onEndChannelEdit = (event: Event) => {
    if (inputRef.current && channelRef.current && triggerRef.current) {
      if (
        ...
        if (inputRef.current.value) { // Accessing value of input element
          mutate({
